...
# Implemetation of RMSNorm compatible for MacOS (MPS/CPU)
class RMSNorm(nn.Module):
    def __init__(self, dim: int, eps: float = 1e-5, device=None, dtype=None):
        super().__init__()
        self.eps = eps
        self.weight = nn.Parameter(torch.ones(dim, device=device, dtype=dtype))
        # AGREGAMOS ESTO: Definir bias como None para compatibilidad
        self.bias = None
...
# Deactivate functions that are required on CUDA 
layer_norm_fn = None
rms_norm_fn = None
...
class Block(nn.Module):
    def __init__(
        ...
        # ---------------------------------------
        # If the optimization functions do not exist in (Mac/MPS), force to shuttdown
        if self.fused_add_norm:
            if layer_norm_fn is None and rms_norm_fn is None:
...
class VisionMamba(nn.Module):
    def __init__(self, 
                 ...
                 fused_add_norm=False, # True using triton/CUDA
        ...
        # Detect if it's on Mac/MPS and optimized functions do not exist
        if layer_norm_fn is None and rms_norm_fn is None:
            print("NOTICE: Automatically disabling fused_add_norm (Mac/MPS Mode)")
            fused_add_norm = False
        # -------------------------------
